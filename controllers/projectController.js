...
exports.getProjects = async (req, res) => {
  try {
    const projects = await Project.findAll(); // recupère tous les projets enregistrés dans db (findAll() est une méthode Sequelize) et await signifie que tu dois attendre une réponse de la db avant de continuer
    res.status(200).json(projects); // ca envoie au client que le status est OK et ca envoie les projets en format JSON
  } catch (error) {
    console.error("Error fetching projects:", error);
    res.status(500).send({ message: "Internal server error" }); // si erreur alors catch dans console.log +  ca envoie le message erreur au client
  }
};
...
exports.createProject = async (req, res) => {
  try {
    ...
    const newProject = await Project.create({ name, description }); // on crée une nouvelle ligne de la base de donnée avec sequelize donc ca insere dans la table project (name et description) les données rajoutés par le client
...
exports.UpdateDroppedItems = async (req, res) => {
  ...
  try {
    ...
    // si ca existe alors on va remplacer les anciens droppedItems par les nouveaux
    if (projectPage) {
